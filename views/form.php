		...
		<div class="col-sm-12">
			...
			<div class="fpbx-container">
				<div class="display full-border">
					<form id="recordings-frm" class="fpbx-submit" name="recordings-frm" action="config.php?display=recordings" method="post" <?php if(isset($data['id'])) {?>data-fpbx-delete="config.php?display=recordings&amp;action=delete&amp;id=<?= $data['id']?>"<?php } ?> role="form">
								...
								<div class="col-md-12">
									<div class="row">
										<div class="form-group">
											...
											<div class="col-md-9">
												<span class="radioset">
													<input type="radio" id="fcode-link-yes1" name="fcode-link" value="yes" class="fcode-item" <?= (isset($data['fcode']) && ($data['fcode'])) ? "checked" : ""?> <?= (isset($data['soundlist']) && (count($data['soundlist']) == 1)) ? "" : "disabled"?>>
													<label for="fcode-link-yes1"><?= _("Yes")?></label>
													<input type="radio" id="fcode-link-no1" name="fcode-link" value="no" class="fcode-item" <?= (!isset($data['fcode']) || !($data['fcode'])) ? "checked" : ""?> <?= (isset($data['soundlist']) && (count($data['soundlist']) == 1)) ? "" : "disabled"?>>
													<label for="fcode-link-no1"><?= _("No")?></label>
												</span>
												<strong><span id="fcode-message" data-message="<?= isset($data['rec_code']) ? sprintf(_("Optional Feature Code %s"),$data['rec_code']) : ""?>"><?= (isset($data['soundlist']) && (count($data['soundlist']) == 1)) ? sprintf(_("Optional Feature Code %s"),isset($data['rec_code']) ? $data['rec_code'] : "") : _("Not supported on compounded or Non-Existent recordings")?></span></strong>
											</div>
										</div>
									</div>
									<div class="row">
								...
								<div class="col-md-12">
									<div class="row">
										<div class="form-group">
											...
											<div class="col-md-9"><input name="fcode_pass" id="fcode_pass" class="form-control fcode-item" value="<?= isset($data['fcode_pass']) ? $data['fcode_pass'] : ""?>" <?= (isset($data['soundlist']) && (count($data['soundlist']) == 1)) ? "" : "disabled"?>></div>
										</div>
									</div>
									<div class="row">
										<div class="col-md-12">
...
<script>var langs = <?= json_encode($langs)?>;var supportedHTML5 = "<?= $supportedHTML5?>";var supportedFormats = <?= json_encode($supported['in'])?>;var supportedRegExp = "<?= implode("|",array_keys($supported['in']))?>";var systemRecordings = <?= $jsonsysrecs?>;var soundList = <?= isset($data['soundlist']) ? json_encode($data['soundlist']) : "{}"?>;var playbackList = <?= isset($data['playbacklist']) ? json_encode($data['playbacklist']) : "[]"?>;</script>
<div id="playbacks">
</div>
